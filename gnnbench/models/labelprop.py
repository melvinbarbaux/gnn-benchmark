...
class LabelPropagation(GNNModel):
    def __init__(self, features, graph_adj, targets, nodes_to_consider, labelled_nodes, prop_type, return_prob):
        ...
        self.initial_predicted_labels = tf.scatter_nd(
            tf.expand_dims(self.labelled_nodes, -1),
            targets[labelled_nodes],
            shape=targets.shape
        )
    ...
    def optimize(self, learning_rate, global_step):
        if self.prop_type == 'vanilla':
            new_predicted_labels = tf.sparse.sparse_dense_matmul(self.graph_adj, self.predicted_labels)
            new_predicted_labels = new_predicted_labels / tf.expand_dims(self.degrees, axis=-1)
            new_predicted_labels *= self._get_labelled_nodes_mask()
            new_predicted_labels += self.initial_predicted_labels
        else:
            new_predicted_labels = (1 - self.return_prob) * tf.sparse.sparse_dense_matmul(self.graph_adj, self.predicted_labels) \
    ...
    def _get_labelled_nodes_mask(self):
        inv_mask = tf.scatter_nd(
            tf.expand_dims(self.labelled_nodes, -1),
            tf.ones([tf.shape(self.labelled_nodes)[0], self.num_classes], dtype=tf.float32),
    ...
    def _preprocess_adj(self, graph_adj):
        self.degrees = tf.constant(np.array(graph_adj.sum(axis=1)).flatten().astype(np.float32))
