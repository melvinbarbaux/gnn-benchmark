...
from gnnbench.data.make_dataset import get_dataset_and_split_planetoid, get_dataset, get_train_val_test_split, get_split_feed_dicts
...
def run_single_split(experiment_name, model_name, dataset, num_training_runs, dataset_source, data_path, metrics,
    ...
    @ex.capture
    def build_dataset(dataset, dataset_source, data_path, standardize_graph, split, _log):
        if dataset_source == 'planetoid' or dataset_source == 'planetoid_random':
            ...
            return get_dataset(dataset, data_path, standardize_graph, _log,
                               split['train_examples_per_class'],
    ...
    @ex.main
    def run_experiment_single_split(split_no, learning_rate, num_epochs, early_stopping_tolerance, early_stopping_criterion,
                                    improvement_tolerance, metrics, alternating_optimization_interval, lr_decay_factor,
                                    lr_decay_steps, report_interval, num_training_runs, _run, _log, _seed, device_id=None):
        ...
        if device_id is not None:
            ...
            tf.config.set_visible_devices([physical_devices[int(device_id)]], 'GPU')
        ...
        for run_no in range(num_training_runs):
            ...
            train_step, early_stopping, learning_rate_placeholder, global_step, init_op = build_train_ops(
                model, early_stopping_tolerance, early_stopping_criterion, improvement_tolerance, _run, _log)
            init_op()
            final_test_metrics = run_train_ops(
                train_step, early_stopping, learning_rate_placeholder, global_step, init_op,
                *split_slug[-4:], metrics, model, learning_rate, num_epochs, early_stopping_criterion,
                alternating_optimization_interval, lr_decay_factor, lr_decay_steps, report_interval, _run, run_no, _log, traces)
